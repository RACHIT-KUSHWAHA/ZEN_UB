...
def generate_pyro_session():
    ...
    with PClient(':memory:', api_id=APP_ID, api_hash=API_HASH) as hellbot:
        print("\nYour ZenuserBot Session Is sent in your Telegram Saved Messages.")
        hellbot.send_message(
            "me",
            f"#ZenUserBOT_MUSIC #ZENUSERBOT_SESSION #PYROGRAM\n\n`{hellbot.export_session_string()}`",
        )


def generate_telethon_session():
    print("\nTelethon Session For ZenUserBot!")
    APP_ID = int(input("\nEnter APP ID here: "))
    API_HASH = input("\nEnter API HASH here: ")
    with TelegramClient(StringSession(), APP_ID, API_HASH) as hellbot:
        ...
        hellbot.send_message(
            "me",
            f"#ZENUSERBOT #ZENBOT_SESSION #TELETHON \n\n`{hellbot.session.save()}`",
        )


def generate_insta_session():
    print("Instagram Session For ZENBot!")
    cl = IClient()
